...
app.get('/app/login',encoder,function(req , res){
  console.log("A request has been received from app");
  MongoClient.connect(url , function(err, db){
  var users = db.collection("users")
    if(err){
      res.send('Some error, rare case try again later');
      ...
      users.find({username:req.body.username,password:req.body.password}).toArray(function(err, docs){
